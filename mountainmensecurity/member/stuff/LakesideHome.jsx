...
const LakesideHome = ({side}) => {
  ...
  <div className='outputStyle'>
      ...
      <div className='cardBlog'>
        <p>{side.publishedAtLake}</p>
        <div>
        <img  src={urlFor(side.imageLake)} alt="lavenderefield" className="footer-banner-image"/>
        </div>
        
      
        <h1>{side.titleLake}</h1>
          ...
          Read more {dropBox
            ...
            {dropBox && (
              <div className='Information1' id='blogs'>
                  <PortableText value={side.lakedetails} components={components} />
...
const components = {
  types: {
    code: side => (
      <pre data-language={side.node.language}>
        <code>{side.node.code}</code>
      </pre>
    )
  }
}

export default LakesideHome